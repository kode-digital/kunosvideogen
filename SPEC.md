...
5. **`capture/specs/demo_walkthrough.spec.ts` built and run against the real demo (2026-08-28)** — a client-requested capture for a sales-demo VO script, separate from the V1 slice (`npm run capture:demo`, not part of `npm run capture`). Verified each of the script's claims live before filming anything (per CLAUDE.md, "if a feature is missing, the script changes"): the CRM section's customer birthday/tier/stock-check claims, the entire Logistics section (`/kenderaan/*` is an internal staff vehicle-**booking** workflow — "government business only", passenger cars — not delivery management; no driver reports, capacity planning, or Maps integration anywhere), the entire Finance section (no single screen matches "photo → data → tag → submit" for invoices; the closest real features are CSV-import classification and general document-search OCR, neither the same shape; named integrations don't exist in the codebase), and the AI-drafts-a-quote finale (HR-scoped only) are all confirmed **not real as scripted** and were skipped. What's real and filmed: Login → Inventory Dashboard (the actual stock-value/low-stock/out-of-stock widgets the script describes, which live at `/inventory`, not the generic post-login `/dashboard`), Inventory item creation + the Supplier Documents OCR flow (a genuine, working analogue of the script's "scan a delivery document" claim, just correctly attributed to Inventory rather than Finance), CRM company view + real quotation creation, and a real HR leave-application submission.
   Getting from "the underlying features are real" to "the recording actually works" needed five more fixes, all confirmed live and now in `capture/lib/`: (a) `guards.ts`'s `<select>`-innerText false positive (Chrome/Firefox return *all* option text for a `<select>`, not just the selected one, so any page with a Company/Deal/Supplier picker failed the guard regardless of what was actually on screen); (b) a second guard false positive where a field label glued directly to its value with no separating space (`"CompanyAurora Hardware Sdn Bhd"`) defeated the space-anchored suffix check — switched to a plain suffix match; (c) `cursor.ts`'s `moveToElement()` not scrolling an off-screen target into view first (a below-the-fold submit button silently no-opped); (d) `moveAndClick()` clicking via raw mouse coordinates rather than the element's own `Locator.click()`, which doesn't wait for a button to actually become clickable (surfaced as two unrelated buttons staying natively `disabled` under headed/Xvfb capture specifically); (e) a **tenant-wide duplicate-file check** on Supplier Documents that had been silently rejecting re-uploads of the same cached synthetic document across repeated runs, which looked exactly like a flaky "stuck disabled button" until the real server response was inspected. `recorder.ts` also no longer orphans Xvfb when ffmpeg fails to start (a real, confirmed-live bug independent of this capture). Output: four ~20–50s clips + sidecars + a manifest under `library/ui/demo_walkthrough/`, no voiceover (the user records that separately).
