...
4. **`boq_scan_to_quote` (§6.6) is real and works end-to-end — with one important scope limit found in live testing (2026-09-03).** Login → create a deal with `boq_enabled` checked (this is a per-deal feature toggle, off by default — BOQ creation against a deal without it fails server-side with a "BOQ is not enabled for this Deal" modal error, not a form validation error) → create a BOQ → upload a document for OCR → review/import → convert to quotation all work against the real demo, and `capture/specs/boq_scan_to_quote.spec.ts` gets through every step up through the OCR review page. But: `BoqScanController@store` (§6.6, §5.2's OCR extraction) is not a generic "scan any site document" feature — it defaults to a single hard-coded `CrmDocumentTemplate` (`code=MAMMO_PREINSTALL_BOQ`), the only template seeded anywhere in the codebase, built from one specific real pre-printed physical form used for **mammography equipment pre-installation** work — a different customer vertical from Kunos's stated hardware/retail/warehousing positioning. A generic warehouse-style document extracted 0 rows against it in a real test. Full evidence in `knowledge/claims.json`'s "cut processing time by over 80%" entry. **This needs a decision from the user before the shot can be finished** — see that claims.json entry for the three options (find another template, use the real Mammo-form scenario honestly, or change the V1 target again).

Other things worth knowing from this same round of live testing, for whoever picks this back up:
- `capture/lib/cursor.ts`'s `moveAndClick`/`moveToElement` had two real bugs, now fixed: (a) it never scrolled the target into view, so a below-the-fold element's computed click coordinates landed off-screen with no error; (b) the final click used a raw `page.mouse.click(x, y)` at a coordinate computed slightly earlier, which silently missed if anything shifted in between. The click now goes through the element's own `.click()` (auto-scrolling, auto-retrying); the eased mouse movement beforehand is cosmetic only.
- `capture/lib/guards.ts`'s entity-name regex originally allowed `\s` (including newlines) between the words it swept into a match, which pulled in unrelated adjacent UI text (a user's name, a field label) sitting on nearby lines and produced false positives. Fixed to same-line whitespace only (`[ \t]+`).
- Generic button selectors are not reliable in this app: the persistent header has its own hidden `type="submit"` buttons (a notifications panel) that sit earlier in the DOM than a page's real form button, and shared CTA classes like `im-btn-primary` are reused by unrelated elements too — both cases make `.first()` silently click the wrong thing with no error. Some primary buttons also don't set an explicit `type="submit"` HTML attribute at all (relying on the browser default), so `[type="submit"]` attribute selectors silently match nothing. What actually works reliably: scope to the specific `<form>` containing a field you just filled and match the button's exact visible text, or use a real `id` when the markup has one (checked directly against `bos-mirror`'s Blade views rather than guessed).
